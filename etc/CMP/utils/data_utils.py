...
def array_crop(array, crop_size):
    pad_h = max(crop_size[0] - array.shape[0], 0)
    pad_w = max(crop_size[1] - array.shape[1], 0)
    pad_h_half = int(pad_h / 2)
    pad_w_half = int(pad_w / 2)
    if pad_h > 0 or pad_w > 0:
        border = ((pad_h_half, pad_h - pad_h_half),(pad_w_half, pad_w - pad_w_half),(0,0))
        array = np.pad(array, border, 'constant')
    hoff = (array.shape[0] - crop_size[0]) // 2
    woff = (array.shape[1] - crop_size[1]) // 2
    return array[hoff:hoff+crop_size[0], woff:woff+crop_size[1], :]
